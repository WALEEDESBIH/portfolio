document.addEventListener("DOMContentLoaded", () => {
  ...
  const PROJECTS = [
    {
      ...
      tags: ["Data Analysis", "Dashboard", "Inventory", "Charts"],
  ...
  async function loadGitHubStats() {
    ...
    if (kpiRepos) kpiRepos.textContent = String(user.public_repos ?? 0);
    if (kpiStars) kpiStars.textContent = String(totalStars);
    if (kpiFollowers) kpiFollowers.textContent = String(user.followers ?? 0);
